...
export class PlanningActivityController {
  constructor(
    private readonly planningActivityService: PlanningActivityService,
  ) {}

  @Post()
  async create(
    @Body() body: CreatePlanningActivityDto,
  ): Promise<ResponseInterfaceWithData<{ newPlanActivity: PlanningActivity }>> {
    return {
      data: {
        newPlanActivity: await this.planningActivityService.create(body),
      },
  ...
  async findAll(): Promise<
    ResponseInterfaceWithData<{ planActivities: PlanningActivity[] }>
  > {
    return {
      data: { planActivities: await this.planningActivityService.findAll() },
  ...
  > {
    return {
      data: {
        planActivities:
          await this.planningActivityService.getPlanningActivitiesByUser(
            classId,
          ),
      },
      message: 'Liste des activités',
  ...
  async findOne(
    ...
    if (!planActivity)
      throw new CustomHttpException(
        `Activité ${planActivity.id} not exist`,
