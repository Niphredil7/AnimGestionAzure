...
export class PlanningController {
  ...
  async findAll(): Promise<
    ResponseInterfaceWithData<{ newPlanning: Planning[] }>
  > {
    return {
      data: { newPlanning: await this.planningService.findAll() },
  ...
  async getNextPlannings(
    ...
    const data = await this.planningService.findNextByClass(
      classId,
      fromDate,
      nb,
    );
  ...
  async findOne(
    ...
    if (!planning)
      throw new CustomHttpException(
        `Planning ${planning.id} not exist`,
