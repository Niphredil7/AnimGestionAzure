...
export class HappenningController {
  ...
  @Post()
  async create(
    @Body() body: CreateHappenningDto,
    @Req() req: IRequestWithPayload,
  ): Promise<ResponseInterfaceWithData<{ newHappenning: Happenning }>> {
    if (!req.user.id)
      throw new BadRequestException("vous n'avez pas accès à ce service");
    return {
  ...
  async findAll(): Promise<ResponseInterfaceWithData<{ users: Happenning[] }>> {
    return {
      data: { users: await this.happenningService.findAll() },
  ...
  async findOne(
    ...
    if (!event)
      throw new CustomHttpException(
        `Event ${JSON.stringify(event)} not exist`,
