...
export class UserHasNotificationController {
  ...
  @Post()
  async create(
    @Body() body: CreateUserHasNotificationDto,
    @Req() req: IRequestWithPayload,
  ): Promise<ResponseInterfaceWithData<{ newUserNotif: UserHasNotification }>> {
    const userNotifExist = await this.userHasNotificationService.findOne(
      body.notificationId,
      req.user.id,
    );
    if (userNotifExist)
    ...
    return {
      data: {
        newUserNotif: await this.userHasNotificationService.create(body),
      },
  ...
  async findOne(
    ...
    const userNotif = await this.userHasNotificationService.findOne(
      notificationId,
      req.user.id,
    );
    if (!userNotif)
      throw new CustomHttpException(
        `UserNotif ${JSON.stringify(userNotif)} not exist`,
  ...
  async update(
    ...
    const userNotification = await this.userHasNotificationService.findOne(
      notificationId,
      req.user.id,
    );
    if (!userNotification) {
      throw new NotFoundException(
        `Notification ${notificationId}, ${req.user.id} not exist`,
      );
    }
    const notifUpdate = await this.userHasNotificationService.update(
      notificationId,
      req.user.id,
      body,
    );
