...
export class UserHasNotificationService {
  ...
  async create(
    body: CreateUserHasNotificationDto,
  ...
  async findOne(
    ...
    userId: string,
  ): Promise<UserHasNotification | null> {
    return await this.prisma.userHasNotification.findUnique({
  ...
  async remove(
    ...
    return await this.prisma.userHasNotification.delete({
      where: { notificationId_userId: { notificationId, userId } },
    });
  }
}
