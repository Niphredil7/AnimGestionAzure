...
export class ActivityController {
  ...
  async findAll(): Promise<
    ResponseInterfaceWithData<{ activities: Activity[] }>
  > {
    return {
      data: { activities: await this.activityService.findAll() },
      message: 'Liste des activity',
    };
  }
  ...
  ): Promise<ResponseInterfaceWithData<{ activities: Activity[] }>> {
    return {
      data: { activities: await this.activityService.findByUser(req.user.id) },
      message: 'Liste des activity d cet user',
    };
  }
  ...
  async findOne(
    ...
    if (!activity)
      throw new CustomHttpException(
        `Student ${JSON.stringify(activity)} not exist`,
