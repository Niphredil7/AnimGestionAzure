...
export class OutingController {
  ...
  async create(
    ...
    if (outing) throw new BadRequestException('Cet sorie est déjà proposée');
    return {
  ...
  async findAll(): Promise<ResponseInterfaceWithData<{ outings: Outing[] }>> {
    return {
      data: { outings: await this.outingService.findAll() },
