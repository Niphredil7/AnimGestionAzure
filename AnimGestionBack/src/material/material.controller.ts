...
export class MaterialController {
  ...
  async findAll(): Promise<
    ResponseInterfaceWithData<{ materials: Material[] }>
  > {
    return {
      data: { materials: await this.materialService.findAll() },
      message: 'Liste des materials',
    };
  }
  ...
  async findOne(
    ...
    if (!material)
      throw new CustomHttpException(
        `Material ${material.id} not exist`,
