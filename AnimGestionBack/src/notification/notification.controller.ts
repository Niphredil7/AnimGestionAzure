...
import {
  ...
  CreateHappenningNotificationDto,
...
export class NotificationController {
  ...
  async createActivityNotification(
    @Body() body: CreateActivityNotificationDto,
  ...
  async createHappenningNotification(
    @Body() body: CreateHappenningNotificationDto,
  ): Promise<ResponseInterfaceWithData<{ newNotification: Notification }>> {
    const { userIds, happenningId, message } = body;
    return {
      data: {
        newNotification:
          await this.notificationService.createHappenningNotification(
            userIds,
            happenningId,
            message,
          ),
  ...
  async findAll(): Promise<
    ResponseInterfaceWithData<{ users: Notification[] }>
  > {
    return {
      data: { users: await this.notificationService.findAll() },
      message: 'Liste des notifs',
    };
  }
