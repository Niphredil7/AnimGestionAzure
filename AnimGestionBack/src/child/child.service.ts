...
import { ActivityForbidenn, Child, Regime } from '@prisma/client';
...
export class ChildService {
  ...
  async create(
    ...
    activityForbiden: ActivityForbidenn = ActivityForbidenn.NONE,
  ): Promise<Child> {
  ...
  async countByGenderByClass(classId: string): Promise<number[]> {
    const girls = await this.prisma.child.count({
      where: { classId, sexe: 'F' },
    });

    const boys = await this.prisma.child.count({
      where: { classId, sexe: 'M' },
    });
  ...
  async update(id: string, body: UpdateChildDto): Promise<Child> {
