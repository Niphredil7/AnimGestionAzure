...
import type { EMoment, PlanningActivity } from "../types/planningActivity";
import type { Favorite } from "../types/activity.type";
...
export default function PlanningModal(props: PlanningModalProps) {
 ...
 const formik = useFormik<IActivityData>({
  ...
  initialValues: {
    ...
    roomId: planningActivity?.activity?.room?.id ?? "",
    categoryMaterialId: planningActivity?.activity?.categoryMaterialId ?? "",
    content: planningActivity?.activity?.content ?? "",
    userId: user?.id || "",
       date: date.toISOString(),

  momentEnum: momentEnum as EMoment,

  favorite: planningActivity?.activity?.favorite ?? (null as unknown as Favorite),
  },
