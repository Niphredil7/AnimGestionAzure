...
export default function Planning({ monday }: PlanningProps) {
  ...
  const momentToEnum: Record<string, EMoment> = {
  ...
  const [selectedMomentEnum, setSelectedMomentEnum] = useState<EMoment | null>(null);
  ...
  dayNumber={selectedDay ? dayToIndex[selectedDay] : 1}
  moment={selectedMomentEnum}
