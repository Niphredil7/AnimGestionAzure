...
class AuthApi {
  ...
  public async refreshTokens(): Promise<RefreshResponse> {
    ...
    try{
      const { data } = await AxiosRefreshClient().post<RefreshResponse>(`https://animgestion-back-dudme8dwhpd4hhfk.francecentral-01.azurewebsites.net/auth/refresh`, null, { headers:{Authorization:'Bearer ' + refreshToken} });
